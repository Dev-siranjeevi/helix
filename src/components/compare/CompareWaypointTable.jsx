...
const WP_COLS = [
  { key:"latitude",label:"Lat" },{ key:"longitude",label:"Lon" },
  { key:"type",label:"Type" },{ key:"format",label:"Format" },{ key:"name",label:"Name" },
];

export default function CompareWaypointTable({ waypointDiff, nameA, nameB, diffOnly = false, filePath, highlights, onHighlightChange, onClearAll }) {
  const diffRows = waypointDiff.filter((r) => r.diff !== "same");
  const sameCount = waypointDiff.filter((r) => r.diff === "same").length;
  const rows = diffOnly ? diffRows : waypointDiff;
  ...
  const allKeys = rows.flatMap((r) => WP_COLS.map((c) => `cwp:${r.ident}.${c.key}`));
  ...
  const renderRows = () => rows.flatMap((r) => {
    ...
    if (r.diff === "added" || r.diff === "removed") {
      ...
      return [<tr key={r.ident} className={`ari-tr diff-row--${r.diff}`}>
        ...
        <td className="ari-td diff-val--same" colSpan={3}>Waypoint {r.diff} — {wp?.latitude} {wp?.longitude}</td>
      </tr>];
    }
    return (r.fields ?? []).filter((f) => f.diff !== "same").map((f) => {
      const ck = `cwp:${r.ident}.${f.field}`;
      return (
        <tr key={`${r.ident}.${f.field}`} className="ari-tr diff-row--changed">
          <td className="ari-td diff-val--same">{r.ident}</td>
          <td className="ari-td diff-val--same">{WP_COLS.find((c) => c.key === f.field)?.label ?? f.field}</td>
  ...
  return (
    <div className="table-section">
      <div className="table-section-label">Waypoints</div>
      <div className="table-section__scroll">
        <table className="ari-table">
          <thead><tr><th className="ari-th">Ident</th><th className="ari-th">Field</th><th className="ari-th"><span className="diff-th--b">New (B)</span><span style={{margin:"0 4px",opacity:0.5}}>→</span><span className="diff-th--a">Old (A)</span></th><th className="ari-th"/></tr></thead>
          <tbody>
            {renderRows()}
            {diffRows.length === 0 && diffOnly && <tr><td colSpan={4} className="ari-td diff-val--same" style={{textAlign:"center", padding:"14px"}}>No waypoint differences</td></tr>}
          </tbody>
        </table>
      </div>
      {!diffOnly && sameCount > 0 && <div className="compare-identical-note">{sameCount} identical waypoint{sameCount !== 1 ? "s" : ""} shown</div>}
